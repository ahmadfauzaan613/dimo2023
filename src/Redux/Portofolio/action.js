...
export const getAllPortofolio = () => async (dispatch) => {
  dispatch(setLoading(true))
  dispatch(setError(null))
  dispatch(allEntity(await getCollection(scope)))
  dispatch(setLoading(false))
}
export const postEntity = (nama_portofolio, gambar) => async (dispatch) => {
  dispatch(setEntity(await createItem(scope, { nama_portofolio, gambar: await fileToDataUrl(gambar) })))
}
...
export const updateEntity = (id, nama_portofolio, gambar) => async (dispatch) => {
  const current = (await getCollection(scope)).find((item) => String(item.id) === String(id))
  dispatch(setEntity(await updateItem(scope, id, { nama_portofolio, gambar: await fileToDataUrl(gambar, current?.gambar) })))
}
